...
describe('sitechecker history', () => {
  ...
  it('should store a summary with diff in the brain', () => {
    const brain = { set: () => undefined, get: () => undefined }
    const Mbrain = sinon.mock(brain)
    Mbrain.expects('get').returns(JSON.parse(JSON.stringify({
      summary: {
        timestamp: 1,
        url: 'http://a.com',
        brokenLinks: [
  ...
  it('should return stored brain data on get', () => {
    ...
    Mbrain.expects('get').returns(JSON.parse(JSON.stringify(data)))
    ...
    // assert
    expect(summary.summary.url).to.deep.equal(data.summary.url, 'summary.url')
    expect(summary.summary).to.deep.equal(data.summary, 'summary')
    expect(summary.diff).to.deep.equal(data.diff, 'diff')
  })
